...
test("acepta E.164 (con o sin '9', con o sin '+') y siempre devuelve el formato sin '9' que espera la Graph API", () => {
  assert.equal(normalizeArgentinePhone("+5491123456789").e164, "541123456789");
  assert.equal(normalizeArgentinePhone("541123456789").e164, "541123456789");
  assert.equal(normalizeArgentinePhone("+54 11 2345-6789").e164, "541123456789");
});

test("acepta formato con prefijo internacional 00", () => {
  assert.equal(normalizeArgentinePhone("0054 9 11 2345-6789").e164, "541123456789");
});
...
test("acepta número local con 0 de larga distancia + 15 de celular (Buenos Aires, área de 2 dígitos)", () => {
  ...
  assert.equal(result.e164, "541123456789");
});
...
test("acepta área de 4 dígitos con 15 (ej. interior del país)", () => {
  ...
  assert.equal(result.valid, true);
  assert.equal(result.e164, "542954123456");
});

test("nunca antepone '9' al código de país — verificado con un envío real (2026-07-11): Meta rechaza el 'to' con '9' para números AR", () => {
  const result = normalizeArgentinePhone("+54 2954 12-3456");
  assert.equal(result.valid, true);
  assert.equal(result.e164, "542954123456");
});
...
test("formatE164ForDisplay antepone '+' para mostrar en UI/logs", () => {
  assert.equal(formatE164ForDisplay("541123456789"), "+541123456789");
});
