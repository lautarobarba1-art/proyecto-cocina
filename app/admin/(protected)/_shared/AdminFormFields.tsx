  ...
  return (
    <div>
        ...
        {rows.map((row, index) => (
          <div key={index} className="flex items-center gap-2">
            <input
              ...
              className="w-full border border-carbon/20 bg-white px-3 py-2 font-sans text-[0.92rem] text-carbon outline-none transition focus:border-terracota"
            />
            <button
              type="button"
              onClick={() => moveRow(index, -1)}
              disabled={index === 0}
              className="px-2 py-1 font-sans text-carbon/50 hover:text-carbon disabled:opacity-30"
              aria-label="Subir"
            >
              ↑
            </button>
            ...
            <button
              ...
              className="px-2 py-1 font-sans text-red-600 hover:underline disabled:opacity-30"
              aria-label="Eliminar"
            >
              ×
            </button>
      ...
      <button
        type="button"
        onClick={addRow}
        className="mt-2 rounded border border-carbon/30 bg-white px-3 py-1.5 font-sans text-[0.78rem] uppercase tracking-wide text-carbon/70 transition hover:bg-carbon/5"
      >
        + Agregar
      </button>
...
export function FormActions({
  ...
  loading: boolean;
  isEdit: boolean;
  entityLabel: "clase" | "evento" | "receta";
  onCancel: () => void;
}) {
  return (
