...
import { dbRowToEventoForm } from "@/lib/admin/eventos-validation";

export const dynamic = "force-dynamic";

export const metadata = {
  title: "Editar · Admin Menesteres",
};
...
export default async function EditarClasePage({ params }: PageProps) {
  ...
  const isEvento = data.category_event === "eventos";
    ...
    <div>
      <header>
        ...
        <h1 className="mt-3 font-display text-3xl font-normal tracking-tightish text-carbon">
          {isEvento ? "Editar evento privado" : "Editar clase"}
        </h1>
        <p className="mt-3 font-body text-[0.92rem] leading-relaxed text-carbon/65">
          {data.title}{" "}
          <span className="text-carbon/45">· {data.date}</span>
        </p>
      </header>
      ...
      <div className="mt-10 max-w-3xl">
        {isEvento ? (
          <EventoFormCliente
            initial={{ id: data.id, ...dbRowToEventoForm(data) }}
          />
        ) : (
          <ClaseFormCliente
            initial={{
